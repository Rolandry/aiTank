...
在线多人坦克对战小游戏，支持 2～4 人自由混战：无尽死斗按时限结算，经典模式由最后存活者获胜。
...
│       ├── collision.ts     # 碰撞检测
│       └── map.ts           # 地图定义
└── tank-battle-assets-1.0/  # 美术资源 2.0 与原始美术 Skill
```
...
| 射击冷却 | 500 毫秒 |
| 子弹上限 | 每人最多 3 颗在场 |
| 胜利条件 | 无尽死斗：时间耗尽时击杀数第一（并列比命中数，仍并列则平局）；经典模式：最后存活玩家获胜 |
| 结束条件 | 无尽死斗 120 秒时间耗尽；经典模式仅剩一名存活玩家 |
| 击杀标记 | 击杀者在坦克上方累积显示与自身颜色匹配的击杀图标 |
| 复活 | 无尽死斗被击杀 3 秒后随机出生点满血复活；经典模式淘汰后不复活；意外断线恢复后继续本局 |
